...
def test_golden_path_full_shape():
    ...
    actions = {(item.action, item.ticker, item.account_id, item.replacement_ticker) for item in recommendations}
    assert ("tax_loss_harvest", "VXUS", "TAX-1", "VEA") in actions
    assert any(item.action == "do_nothing_due_to_tax_cost" and item.ticker == "VTI" for item in recommendations)
    assert any(item.action == "relocate" and item.ticker == "BND" for item in recommendations)
